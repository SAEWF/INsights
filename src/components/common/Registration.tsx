...
function RegistrationPage(props: any) {
    ...
    const RegisterUser = async () => {
      ...
      else if(walletID !== system.tzPublicKey){
        document.querySelector('.walletcheck')!.innerHTML = "Please dont tamper wallets value !!";
        return;
      }
      ...
      await docRef.get().then(async (doc)=>{
        if(doc.exists){
          if(doc.data()!.display===undefined){
            ...
            var data1 = {...formData,"avatar":url, "walletAddress":walletID};
            docRef.update(data1);
            setSuccess(true);
          }
          else 
    ...
    if(system.status !== 'WalletConnected'){
      return(
        <>
          ...
          <div style={{display: 'flex',alignItems: 'center',justifyContent: 'center', color: 'cyan', fontSize: '30px', marginTop: '15%'}}>
            Connect to wallet for registering .
          </div>
          </Container>
          </Flex>
        </>
      ...
      <Box p={6} maxWidth="100%" borderWidth={3} borderRadius={10} boxShadow="lg">
            ...
            <Form id="myform" onSubmit={handleSignup} onChange={handleChange}>
              ...
              <div className="row align-items-center justify-content-center">
              <FormGroup className="col-md-8 col-12">
                  <Input value={system.tzPublicKey} 
                    isInvalid type="text" 
                    errorBorderColor="cyan.300" 
                    variant="filled"
                    disabled
